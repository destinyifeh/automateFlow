import logo from "@/assets/fby-header-logo.png";
import { Button } from "@/components/ui/button";
import { AnimatePresence, motion } from "framer-motion";
import { Briefcase, Heart, HelpCircle, Home, Info, Menu, PlusCircle, X } from "lucide-react";
import React, { useState } from "react";
import { AboutUsSection } from "./sections/AboutUsSection";
import { CareerExploreSection } from "./sections/CareerExploreSection";
import { FeaturesComingSoonSection } from "./sections/FeaturesComingSoonSection";
...
const navigationItems = [
  { label: "Home", active: true, href: "#home", icon: Home, showOnDesktop: true },
  { label: "About", active: false, href: "#about", icon: Info, showOnDesktop: true },
  { label: "Features", active: false, href: "#features", icon: Heart, showOnDesktop: true },
  { label: "Business", active: false, href: "#business", icon: Briefcase, showOnDesktop: true },
  { label: "Join Waitlist", active: false, href: "#home", icon: PlusCircle, showOnDesktop: false },
  { label: "Career", active: false, href: "#business", icon: Briefcase, showOnDesktop: false },
  { label: "Support", active: false, href: "#footer", icon: HelpCircle, showOnDesktop: false },
];
...
export const MacbookPro = (): JSX.Element => {
  ...
  const handleNavClick = (
    ...
    if (element) {
      console.log("__ANIMA_DBG__ Navigation clicked:", href);
  ...
  return (
    <div
      className="bg-[#fff7ec] overflow-hidden w-full min-w-[320px] relative selection:bg-[#8d5241] selection:text-[#fff2da]"
      data-model-id="1053:980"
    >
      ...
      <nav className="relative z-50 flex items-center justify-between px-6 lg:px-20 pt-8 lg:pt-14 pb-8 max-w-[1568px] mx-auto gap-4">
        ...
        <div className="hidden lg:flex items-center gap-2.5 bg-[#a67b5b38] rounded-[30px] px-2.5 py-2 animate-fade-in [--animation-delay:200ms]">
          {navigationItems.filter(item => item.showOnDesktop).map((item, index) => (
            <a
              key={index}
              href={item.href}
      ...
      >
        ...
        <motion.section
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.8 }}
          className="flex justify-center px-6 lg:px-20 py-16 w-full max-w-[1568px]"
        >
        ...
        {/* 9. Partner With Us / Footer Section */}
        <motion.section
          initial={{ opacity: 0, y: 50 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        ...
        {/* 10. Be first to try / Final Site Footer */}
        <motion.section
          initial={{ opacity: 0, y: 50 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
